          ...
          <div className="flex-1 overflow-y-auto p-3 space-y-4 select-none">
            {slides.map((slide, idx) => (
              <div
                ...
                className={`mx-2 rounded-xl cursor-pointer transition-all duration-200 group ${activeSlideIdx === idx
                    ? 'ring-2 ring-indigo-500 ring-offset-2 ring-offset-[#131316] scale-[1.02]'
                    : 'hover:scale-[1.01] hover:ring-1 hover:ring-white/20'
                  }`}
              >
                <div className="relative w-full aspect-video">
                  {renderMiniPreview(slide)}
                  <div className="absolute -bottom-2 -left-2 bg-blue-500 text-white text-xs font-bold rounded-md px-2 py-0.5 shadow-md border border-blue-400 z-10 flex items-center justify-center">
                    {idx + 1}
                  </div>
                </div>
              </div>
